...
uint8_t DS18B20_Start(void)
{
	...
	if (!(HAL_GPIO_ReadPin(DS18B20_SENSOR_GPIO_Port, DS18B20_SENSOR_Pin)))
		Response = INIT_SUCCESS; // if the pin is low i.e the presence pulse is detected
	else
		Response = INIT_FAIL;
...
{
	uint8_t Presence = INIT_FAIL;
	uint8_t Temp_byte1, Temp_byte2;
	uint16_t TEMP;
	float Temperature;
	Presence = DS18B20_Start();
	while(Presence!=INIT_SUCCESS)
	{
...
void DHT11_Start(void)
{
	...
	delay(20000);													// wait for 18ms		1.2ms for DHT22 10ms
...
uint8_t DHT11_Check_Response(void)
{
	...
	if (!(HAL_GPIO_ReadPin(DHT11_SENSOR_GPIO_Port, DHT11_SENSOR_Pin))) // if the pin is low
	{
		...
		if ((HAL_GPIO_ReadPin(DHT11_SENSOR_GPIO_Port, DHT11_SENSOR_Pin)))
			Response = INIT_SUCCESS; // if the pin is high, response is ok
		else
			Response = INIT_FAIL; // 255
	}
